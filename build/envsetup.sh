...
function alpharemote()
{
    ...
    git remote rm alpha 2> /dev/null
    ...
    then
        ...
        local PFX="alphadroid-project/"
    else
        local PROJECT=$REMOTE
    fi

    local ALPHA_USER=$(git config --get review.review.alphadroid-project.org.username)
    if [ -z "$ALPHA_USER" ]
    then
        git remote add alpha ssh://review.alphadroid-project.org:29418/$PFX$PROJECT
    else
        git remote add alpha ssh://$ALPHA_USER@review.alphadroid-project.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'alpha' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/alphadroid-project/$PROJECT
...
function alphagerrit() {
    ...
    local user=`git config --get review.review.alphadroid-project.org.username`
...
function alpharebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "AlphaDroid Gerrit Rebase Usage: "
        echo "      alpharebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.alphadroid-project.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function repopick() {
    T=$(gettop)
    $T/vendor/alpha/build/tools/repopick.py $@
}
